...
for(i in 1:length(TERMS)){
    ...
    id2012 <- which(as.Date(world$date) > as.Date("2011-12-31") & as.Date(world$date) <= as.Date("2012-12-31"))
    ...
    polygon(c(min(id2012), min(id2012), max(id2012), max(id2012)), c(as.numeric(t.test(world$hits[id2012])$conf.int)[1:2], as.numeric(t.test(world$hits[id2012])$conf.int)[2:1]), col=grey(.75), lwd=1, border=F, density=30)
