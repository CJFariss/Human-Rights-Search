## google_search_cy_analysis_AI_search.R
##########################################################################
##
...
if(lowsearch_option==TRUE) test_dat <- readRDS("Data_output/combined_gsearch_dat_list_merged_AI.RDS")
if(lowsearch_option==FALSE) test_dat <- readRDS("Data_output/combined_gsearch_dat_list_merged_lowsearch_AI.RDS")
...
test_dat_language_pooled[[1]] <- rbind(test_dat[[1]], test_dat[[5]], test_dat[[9]], test_dat[[13]])
test_dat_language_pooled[[2]] <- rbind(test_dat[[2]], test_dat[[6]], test_dat[[10]], test_dat[[14]])
test_dat_language_pooled[[3]] <- rbind(test_dat[[3]], test_dat[[7]], test_dat[[11]], test_dat[[15]])
test_dat_language_pooled[[4]] <- rbind(test_dat[[4]], test_dat[[8]], test_dat[[12]], test_dat[[16]])
...
for(j in 1:4){
  ...
  if(attention_rate==FALSE & NGO_amnesty==TRUE) LABELS <- c("FDI Inflows", "NGO Presence (Count)", "HR Treaty Ratifications", "GDP Growth", "HR Violations", "Internet Censorship", "English/French (Amnesty International)", "Spanish (Amnistia Internacional)", "Portuguese (Anistia Internacional)", "Arabic (Munazamat Aleafw Alduwalia)")
  ...
  if(attention_rate==FALSE & NGO_amnesty==FALSE) LABELS <- c("FDI Inflows", "NGO Presence (Count)", "HR Treaty Ratifications", "GDP Growth", "HR Violations", "Internet Censorship", "English/French (Amnesty International)", "Spanish (Amnistia Internacional)", "Portuguese (Anistia Internacional)", "Arabic (Munazamat Aleafw Alduwalia)")
  ...
  tab_output <- star_insert_row(tab_output, paste("R-squared  &", round(r_squared[[i]][1],3), "&", round(r_squared[[i]][2],3), "&", round(r_squared[[i]][3],3),"\\\\ "), insert.after=36) 
  
  tab_output <- star_insert_row(tab_output, paste("sample size  &", unit_n[1], "&", unit_n[2], "&", unit_n[3],"\\\\ "), insert.after=37) 
